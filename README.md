...
A Source 2 plugin to reveal all console commands and variables that are marked as hidden or development-only in Deadlock.

Based on [cvar-unhide-s2](https://github.com/saul/cvar-unhide-s2).
...
1. **Extract the contents of the ZIP archive to the game folder**:

   - 📂 `$STEAM\steamapps\common\Deadlock\game\citadel`

   After extraction there should be a `cvar_unlocker` folder inside the `citadel` folder, e.g. `Deadlock\game\citadel\cvar_unlocker\...`

1. **Update the `game\citadel\gameinfo.gi` file**: \
   Around line 72, add the following search paths.
   ...
   This tells the engine to load the plugin before loading Deadlock.
...
- Remove the `Game	citadel/cvar_unlocker` line from the `gameinfo.gi` file or rename the `cvar_unlocker` folder.
